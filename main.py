...
# Connect to the database and load data into pandas datafram
...
class PlotUpdateFrame(ctk.CTkFrame):
    def __init__(self, master, plot_frame):
        ...
        self.check_mean = ctk.CTkCheckBox(self, text='Add Mean',
                                          variable=self.check_mean_var,
                                          onvalue='on',
                                          offvalue='off')

        self.check_std_var = ctk.StringVar(value='off')
        self.check_std = ctk.CTkCheckBox(self, text='Standard Deviation (1σ)',
                                         variable=self.check_std_var,
                                         onvalue='on',
                                         offvalue='off')

        self.check_2std_var = ctk.StringVar(value='off')
        self.check_2std = ctk.CTkCheckBox(self, text='Standard Deviation (2σ)',
    ...
    def update_plot(self):
        ...
        if std_tick == 'on':
            self.plot_frame.ax.axhspan(mean - stdd, mean + stdd, facecolor='b', alpha=0.25,
                                       label='Standard Deviation (1σ)')
        if std2_tick == 'on':
            self.plot_frame.ax.axhspan(mean - stdd * 2, mean + stdd * 2, facecolor='b', alpha=0.15,
...
class StatusBarFrame(ctk.CTkFrame):
    def __init__(self, master):
        super().__init__(master, border_width=0,
                         fg_color='#151515',
                         bg_color='#151515')

        font = ctk.CTkFont('Windows', size=16)
        self.status_bar = ctk.CTkLabel(self, text='Program is ready! Enter a serial number to begin...',
                                       text_color='darkgrey',
                                       font=font,
                                       pady=15,
                                       padx=10)
...
class MainApp(ctk.CTk):
    def __init__(self, *args, **kwargs):
        ...
        self.plot_update_frame = PlotUpdateFrame(self, self.plot_frame)
    ...
    def plot_button(self):
        ...
        try:
            ...
            self.statusbar_frame.status_bar.configure(text=f'Data for the {serial} is ready')
        except IndexError:
        ...
        self.plot_frame.ax.set_title(f'Measure {chosen_parameter} of the lasers in the database')
